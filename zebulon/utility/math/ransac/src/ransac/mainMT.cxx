...
int  main(int argc, char* argv[])
{
  ...
  RANSACMT<Point2D,double> lineRansac;

              //by default the number of threads used by the RANSAC 
              //implementation is equivalent to the number of cores 
              //the user can change it to be any number in [1,#cores], 
              //next two lines
  //unsigned int numberOfThreads = 2;
  //lineRansac.setNumberOfThreads(numberOfThreads);

  clock_t startTime = clock();

	double usedData = lineRansac.compute(lineParameters, 
		                                   &lpEstimator , 
										                   pointData,
