...
SET(Zebulon_SENSORS_LASER_PACKAGES 
		...
		hokuyo
		hokuyourg)
SET(Zebulon_SENSORS_INS_PACKAGES 
		gx3)
...
SET(Zebulon_SENSORS_LASER_PACKAGE_DIRS 
			...
			${Zebulon_ROOT_DIR}/sensors/laser/hokuyourg)
SET(Zebulon_SENSORS_INS_PACKAGE_DIRS
                        ${Zebulon_ROOT_DIR}/sensors/ins/gx3)
SET(Zebulon_SENSORS_THERMAL_PACKAGE_DIRS
                        ${Zebulon_ROOT_DIR}/sensors/thermal/)
SET(Zebulon_SENSORS_DEPTH_PACKAGE_DIRS
                        ${Zebulon_ROOT_DIR}/sensors/depth/keller)
SET(Zebulon_SENSORS_ZMCU_PACKAGE_DIRS
                        ${Zebulon_ROOT_DIR}/utility/jaus/zmcu)
...
SET(Zebulon_PACKAGE_DIRS
		...
		${Zebulon_ROOT_DIR}/utility/AI/statemachine
                ...
                ${Zebulon_ROOT_DIR}/sensors/depth/keller
                ${Zebulon_ROOT_DIR}/utility/jaus/zmcu
                ${Zebulon_ROOT_DIR}/utility/threading/pipetree
		)
...
MACRO(ZEBULON_ADD_SUBDIRECTORY PACKAGE BINARY_DIR)
	...
	IF(WIN32 AND ${PACKAGE} MATCHES cvcapture)
		...
		ENDIF()
		link_directories(${Zebulon_ROOT_DIR}/sensors/video/cvcapture/lib/)
		...
		INCLUDE_DIRECTORIES(${Zebulon_ROOT_DIR}/sensors/video/cvcapture/include
                            ${Zebulon_ROOT_DIR}/sensors/video/cvcapture/ext/fw1394/include)
		...
		FOREACH(PKG ${Zebulon_PACKAGES})
			...
			IF(${PKG} MATCHES ${PACKAGE})
				...
				ELSEIF(${PACKAGE} MATCHES laserfactory)
					...
					ENDFOREACH()
					INCLUDE_DIRECTORIES(${Zebulon_INCLUDE_DIRS})
				ELSEIF(${PACKAGE} MATCHES cvcapture)
				    SET(Zebulon_INCLUDE_DIRS 
						    ${Zebulon_INCLUDE_DIRS} 
						    ${Zebulon_ROOT_DIR}/sensors/video/cvcapture/include)
				ENDIF()
				...
				# Special case for cvcapture
				IF(${PKG} MATCHES cvcapture)
					include(${Zebulon_${PKG}_ROOT_DIR}/build/cmake/libcvcapture/FindPylon.cmake)
					link_directories(${Pylon_LIBRARY_DIRS})
				ENDIF(${PKG} MATCHES cvcapture)
...
MACRO(ZEBULON_ADD_SENSORS_INS_CMAKELISTS PACKAGE BINARY_DIR)
        ZEBULON_ADD_SUBDIRECTORY(${PACKAGE} ${BINARY_DIR})
ENDMACRO(ZEBULON_ADD_SENSORS_INS_CMAKELISTS)

MACRO(ZEBULON_ADD_SENSORS_THERMAL_CMAKELISTS PACKAGE BINARY_DIR)
        ZEBULON_ADD_SUBDIRECTORY(${PACKAGE} ${BINARY_DIR})
ENDMACRO(ZEBULON_ADD_SENSORS_THERMAL_CMAKELISTS)

MACRO(ZEBULON_ADD_SENSORS_DEPTH_CMAKELISTS PACKAGE BINARY_DIR)
        ZEBULON_ADD_SUBDIRECTORY(${PACKAGE} ${BINARY_DIR})
ENDMACRO(ZEBULON_ADD_SENSORS_DEPTH_CMAKELISTS)
