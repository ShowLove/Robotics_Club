...
int main(){
  ...
  //Max scan size
  int scanEnd=1080;      //end of the scan
  ...
  if (hokuyoReader.Connect(device,baud)){
    ...
    std::cout << (hokuyoReader.GetAngleMax()-hokuyoReader.GetAngleMin())/hokuyoReader.GetAngleRes() << std::endl;
  ...
  for (int i=0;i<50;i++){
    ...
    else
    {
      ...
      static FILE *outstream=fopen("hl.txt","w");
      for(int j=0;j<scanEnd;j++)
      {
            fprintf(outstream,"%u ",data[j]);
      }
      fprintf(outstream,"\n");
      fflush(outstream);
    }
