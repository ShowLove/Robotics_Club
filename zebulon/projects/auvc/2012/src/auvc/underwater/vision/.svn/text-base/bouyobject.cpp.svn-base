...
IplImage* BouyObject::ColorMask(const IplImage * imgIn) const
{
    IplImage * imgOut = cvCreateImage(cvGetSize(imgIn),IPL_DEPTH_8U, 1);
    IplImage * temp = cvCreateImage(cvGetSize(imgIn),IPL_DEPTH_8U, 1);
...
IplImage* BouyObject::EdgeMask(const IplImage * imgIn) const
{
...
IplImage* BouyObject::SegmentationMask(const IplImage * imgIn) const
{
    IplImage * imgOut = cvCreateImage(cvGetSize(imgIn),IPL_DEPTH_8U, 1);
...
//    IplImage * segsum = cvCreateImage(cvGetSize(imgIn),IPL_DEPTH_8U, 1);
    ...
    int n_comp = comp->total;
    if(mDebug) cvShowImage("Scratch", scratch);
    if(mDebug) cvShowImage("Hist", hist);
    std::list<CvBox2D> blobList;
...
IplImage* BouyObject::GetMask(const IplImage * imgIn, IplImage * debugOut) const
{
    ...
    if(mDebug) cvShowImage("Seg2 Mask", mask);
    cvPyrUp(mask,result);
...
//    IplImage * tempImage  = TemplateMask(imgOutSmall, threshold, mBouyTemplate);
