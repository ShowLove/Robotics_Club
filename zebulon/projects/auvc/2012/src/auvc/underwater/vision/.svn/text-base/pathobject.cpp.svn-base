...
IplImage * PathObject::TemplateMask(const IplImage * imgIn, const IplImage * threshold, const IplImage * tmplt) const
{
    ...
    blobList =  blobList = Zebulon::Vision::VisionUtils::GetBlobBoxes(threshold,mMinNoiseSizePercent,.95,true);
    IplImage * crop;
    CvFont font;
...
//    if(mDebug) cvShowImage("Near Hist", imgOut);
    cvCalcBackProject(planes,temp,mFarHist);
//    if(mDebug) cvShowImage("Far Hist", temp);
    VisionUtils::CombineMasks(imgOut,temp,imgOut);
...
//    }
    ...
    if(mDebug) cvShowImage("final",imgOut);
...
std::list<CvBox2D> PathObject::GetBounding(const IplImage * imgIn, IplImage * debugOut) const
{
    ...
    if(imgOut1)
    {
        blobList = Zebulon::Vision::VisionUtils::GetBlobBoxes(imgOut1,mMinNoiseSizePercent,.95);
        if (debugOut)
        {
            for(std::list<CvBox2D>::iterator it = blobList.begin(); it != blobList.end(); it++)
            {
...
std::vector<Vision::Return> PathObject::GetVisionReturn(const IplImage * imgIn, IplImage * debugOut) const
{
    ...
    for(std::list<CvBox2D>::iterator it = blobList.begin(); it != blobList.end(); it++)
    {
        ...
        result.mValid = true;
//        result.mAngle = it->angle;
        result.mAngle = VisionUtils::GetAngle(*it);
        //result.mAngle = CxUtils::Orientation::AddToAngle(result.mAngle,180,false);
        if(!foundone)
        {
            foundone = true;
            right = result;
