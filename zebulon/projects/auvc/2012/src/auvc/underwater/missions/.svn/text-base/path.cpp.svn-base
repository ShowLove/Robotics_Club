...
double PathFinder::ImageWidth = 800;
double PathFinder::ImageHeight = 600;
...
/** Gets all visible paths, in order of confidence */
std::vector<Path> PathFinder::GetPaths(const double currentYaw, IplImage* image, IplImage * output)
{
    std::vector<Path> paths;
    ...
    for (int i=0; i<paths.size(); i++)
    {
        paths[i].mAngle = CxUtils::Orientation::AddToAngle(currentYaw, paths[i].mAngle,false);
    }
    // List needs to be sorted at some point...
    return paths;
}
...
std::vector<Path> PathFinder::GetPathsDavid(const double currentYaw, IplImage* image, IplImage * output)
{
    mMutex.Lock();
    PathFinder::ImageWidth = image->width;
    PathFinder::ImageHeight = image->height;
    ...
    for (iter = objects.begin(); iter != objects.end(); iter++)
    {
        ...
        p.mI = iter->mCenterI;
        p.mJ = iter->mCenterJ;
        p.mAngle = CxUtils::Orientation::AddToAngle(currentYaw, iter->mAngle, false);
        p.mArea = iter->mArea;
        paths.push_back(p);
    }
...
bool PathFinder::StepPath(const Path* path, const Controls input, Controls &output)
{
        ...
        && path->mJ < PathFinder::ImageHeight/2+mCenterJThresh)
    {
        ...
        if(fabs(AI::Utility::AngleDiff(input.mYaw, path->mAngle)) < mAngleCloseThresh)
        {
            ready = true;
        }
    }
...
/** Align to the given path */
bool PathFinder::StepPath(const Path* path, double const inY, double &outY, double &outFT, double &outLT)
{
    bool ready = false;
        ...
        && path->mJ < PathFinder::ImageHeight/2+mCenterJThresh)
    {
        ...
        if(fabs(AI::Utility::AngleDiff(inY, path->mAngle)) < mAngleCloseThresh)
        {
            ready = true;
        }
    }
    else
    {
        //output.mAxialThrustEnable = true;
        //output.mAxialThrust = (PathFinder::ImageHeight/2 - path->mJ)*mAxialPixelScale;
        outFT = (PathFinder::ImageHeight/2 - path->mJ)*mAxialPixelScale;

        //output.mLateralThrustEnable = true;
        //output.mLateralThrust = (path->mI - PathFinder::ImageWidth/2)*mLatPixelScale;
        outLT = (path->mI - PathFinder::ImageWidth/2)*mLatPixelScale;
    }
    mMutex.Unlock();
